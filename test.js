...
suite("sqlite3 file", async () => {
    test("file creation", async (t) => {
        ...
        t.assert.strictEqual(
            await access("./test.sqlite", constants.F_OK),
            undefined
        );
...
suite("sqlite3 ddl", async () => {
    test("column ddl valid types", async (t) => {
        ...
        t.assert.strictEqual(
            db._getColumnDdl("test", { type: "NULL" }),
            '"test" NULL'
        );
        t.assert.strictEqual(
            db._getColumnDdl("test", { type: "INTEGER" }),
            '"test" INTEGER'
        );
        t.assert.strictEqual(
            db._getColumnDdl("test", { type: "REAL" }),
            '"test" REAL'
        );
        t.assert.strictEqual(
            db._getColumnDdl("test", { type: "TEXT" }),
            '"test" TEXT'
        );
        t.assert.strictEqual(
            db._getColumnDdl("test", { type: "BLOB" }),
            '"test" BLOB'
        );
    });
    ...
    test("column ddl not null true", async (t) => {
        ...
        t.assert.strictEqual(
            db._getColumnDdl("test", { type: "INTEGER", notNull: true }),
    ...
    test("foreign key ddl columns array", async (t) => {
        ...
        t.assert.strictEqual(
            db._getForeignKeyDdl({
                columns: ["id"],
                references: { table: "test", columns: ["id"] },
            }),
    ...
    test("create table ddl column", async (t) => {
        ...
        t.assert.strictEqual(
            db._getTableDdl("test", { columns: { id: { type: "INTEGER" } } }),
        ...
        t.assert.strictEqual(
            db._getTableDdl("test", {
                ...
                foreignKeys: [
                    {
                        columns: "id",
                        references: { table: "test2", columns: "id" },
                    },
                ],
            }),
...
suite("sqlite apply data def", () => {
    ...
    test("drop table", async (t) => {
        ...
        t.assert.strictEqual(
            db._getTableCacheKey("test") in db._columnTypeCache,
            false
        );
    });
});
...
suite("data manipulation", async () => {
    ...
    test("get col types", async (t) => {
        t.assert.deepEqual(await db._getColumnTypes("test"), {
            id: "INTEGER",
            name: "TEXT",
        });
    });
    ...
    test("insert simple", async (t) => {
        ...
        t.assert.deepEqual(await db.query('SELECT * FROM "test"'), [
            { id: 1, name: "one" },
        ]);
    });
    ...
    test("select [id,name]", async (t) => {
        await db.insert("test", { id: 1, name: "one" });
        t.assert.deepEqual(await db.select(["id", "name"], "test"), [
            { id: 1, name: "one" },
        ]);
    });
    test("select [id,name] from {main.test}", async (t) => {
        await db.insert("test", { id: 1, name: "one" });
        t.assert.deepEqual(
            await db.select(["id", "name"], { table: "test", schema: "main" }),
            [{ id: 1, name: "one" }]
        );
    });
    test("select [id,name] from [main,test]", async (t) => {
        await db.insert("test", { id: 1, name: "one" });
        t.assert.deepEqual(await db.select(["id", "name"], ["main", "test"]), [
            { id: 1, name: "one" },
        ]);
    });
    test("select id where id=1", async (t) => {
        await db.insert("test", { id: 1, name: "one" });
        t.assert.deepEqual(
